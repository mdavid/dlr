...
namespace ToyScript.Parser {
    class ToyParser {
        ...
        private Statement ParseExpressionStatement() {
            ToyExpression expr = ParseExpression();
        ...
        private Statement ParseVarStatement() {
            ...
            if (MaybeEatToken(TokenKind.Equal)) {
                ToyExpression value = ParseExpression();
                MaybeEatToken(TokenKind.SemiColon);
        ...
        private Statement ParseWhileStatement() {
            ...
            ToyExpression test = ParseExpression();
        ...
        public ToyExpression ParseExpression() {
            ToyExpression left = ParsePostfixExpression();
        ...
        private ToyExpression FinishAssignment(ToyExpression left) {
            EatToken(TokenKind.Equal);
            ToyExpression right = ParseExpression();
            return new Assignment(new SourceSpan(left.Start, right.End), left, right);
        }

        private ToyExpression FinishBinaryExpression(Operator op, ToyExpression left) {
            NextToken();
            ToyExpression right = ParseExpression();
            return new Binary(new SourceSpan(left.Start, right.End), op, left, right);
        }

        ToyExpression ParsePostfixExpression() {
            ToyExpression expr = ParsePrimaryExpression();
            return FinishExpressionTerminal(expr);
        }

        ToyExpression ParsePrimaryExpression() {
            Token tok = NextToken();
        ...
        ToyExpression ParseMemberExpression() {
            ToyExpression expression = ParsePrimaryExpression();
        ...
        private ToyExpression ParseNew(SourceSpan start) {
            ToyExpression constructor = ParseMemberExpression();
            EatToken(TokenKind.OpenParen);

            Token end;
            List<ToyExpression> arguments = new List<ToyExpression>();
            for(;;) {
                end = PeekToken();
        ...
        private ToyExpression FinishExpressionTerminal(ToyExpression expr) {
            for (; ; ) {
                Token tok = PeekToken();
                switch (tok.Kind) {
                    case TokenKind.Dot:
        ...
        private ToyExpression ParseMember(ToyExpression expr) {
            EatToken(TokenKind.Dot);
            Token name = ParseName();
            return new Member(new SourceSpan(expr.Start, name.End), expr, name.Image);
        }

        private ToyExpression FinishCall(ToyExpression target) {
            EatToken(TokenKind.OpenParen);

            List<ToyExpression> args = new List<ToyExpression>();
        ...
        private ToyExpression FinishIndex(ToyExpression target) {
            EatToken(TokenKind.OpenBracket);
            ToyExpression index = ParseExpression();
        ...
        public Statement ParseInteractiveStatement() {
            if (PeekToken().Kind != TokenKind.EOF) {
                ...
                if (es != null) {
                    ToyExpression expr = es.Expression;
